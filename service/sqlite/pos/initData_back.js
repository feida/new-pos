...
syncDataUtil.InitPosConfig = function(callback) {
    let obj = new Deploy(config.api_url);
    obj.path = obj.path + "/newpos/shop_detail/server/address";
    ...
    httpClient.get(obj,{},(err,data)=>{
        ...
        let posConfig = {
            ...
            apiUrl: JSON.stringify({
                url: resultData.pos_http_url,
                path: resultData.http_path,
                key: config.api_url.key || null,
                updateTime: new Date().getTime(),
                brandId: config.api_url.brand_id,
                brandId: config.api_url.shopId
            }),
            javaHttpUrl: JSON.stringify({
                url: resultData.pos_web_url,
                path: "/pos/LocalPosSyncData"
            }),
            wsUrl: JSON.stringify({
                url: resultData.pos_mqtt_url
            }),
            ...
            mqttConfig: JSON.stringify({
                receivePlatform: resultData.receive_platform,
                publishThemeResto: resultData.publish_theme_resto,
                publishThemePosAdmin: resultData.publish_theme_pos_admin
            })
        }
        const model = sqlite.model('tb_pos_config');
        model.sync({force: true}).then(()=> {
            model.build(posConfig).save().then((result)=> {
                console.log('init tb_pos_confg success');
                callback();
            });
        });
...
syncDataUtil.allData = function (tableState, apiUrl, callback) {
    let that = this;
    if (tableState == 1) {
